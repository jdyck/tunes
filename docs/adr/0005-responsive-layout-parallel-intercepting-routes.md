...
**Status: Superseded by [ADR-0010](0010-responsive-browse-layout-hybrid-parallel-routes.md).**
...
We decided to implement this with Next.js App Router **parallel routes** (`@list` / `@detail` slots composed by a shared layout) plus **intercepting routes**, so a single URL (e.g. `/song/5/recording/12`) drives both experiences: on desktop, navigating to a song or recording intercepts the link and renders it into the `@detail` slot alongside the still-visible list; on mobile (or on direct load/refresh, where there's nothing to intercept from), the same URL renders as an ordinary full-page navigation. One route tree, no duplicated pages, and the URL stays deep-linkable in both cases. The add-song/add-recording forms are expected to reuse the same intercepting-route technique for their modal-on-desktop / bottom-sheet-on-mobile treatment.
