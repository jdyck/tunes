...
**`src/lib` holds anything effectful or stateful** (I/O, fetch wrappers, supabase access, singletons, config); **`src/utils` holds pure functions only**. The rule is deliberately mechanical — an agent or future self can apply it without judgment: if it fetches, talks to supabase, or holds state, it's `lib`. Applying it revealed that nearly everything then in `utils` was effectful (only `ytmusic.ts` was pure), which is exactly the drift the rule exists to prevent.
