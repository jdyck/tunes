...
**`src/lib` holds anything effectful or stateful** (I/O, fetch wrappers, supabase access, singletons, config); **`src/utils` holds pure functions only**. The rule is deliberately mechanical — an agent or future self can apply it without judgment: if it fetches, talks to supabase, or holds state, it's `lib`. The earlier `ytmusic.ts` helper was pure when this decision was made, but it later became a stateful/networked provider client; its current placement under `src/utils` is a known violation to correct, not an exception to the rule (see [code-organization.md](../direction/code-organization.md)).
