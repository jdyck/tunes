...
The schema is in a compatibility rollout: `artists` is the shared identity, `artist_user_data` is private, and target Song/Recording credit surfaces exist, while the Song-credit storage still mirrors legacy `people` / `song_writers` until contraction. Build browsing only on the target Artist-backed surfaces. The completed contract must:

- retain `artists` as the single shared canonical identity;
- remove the temporary Person mirror without losing Artist-backed Song credits;
- retain private notes and planned personal tags only in `artist_user_data` (at most one private row per User/Artist); and
- retain structured Recording performer credits against the same Artist identity.
