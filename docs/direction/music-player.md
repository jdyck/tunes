...
- **`usePlayer()`** hook, exported from the same file, is how any page starts playback: `usePlayer().play(playable)`. `play` takes a provider-oriented `Playable` (`{ name, youtubeVideoId, artist?, kind? }`) rather than a full `Recording`, since YouTube search results without a saved `user_recording_data` relationship need to play too. Saved Recording callers select the preferred associated YouTube Item and construct the same payload.
...
- **Always above transient UI.** The persistent player bar and its visible video/artwork surface use the highest application layer so playback controls remain visible and interactive over modal dialogs.
...
- When the active `Playable` is a YouTube search result rather than a Recording saved by the current User, offer an obvious path to save it for the Song.
...
- Provide a genuinely compact/minified player state, not just the existing optional cover-art/video visibility. Its always-visible controls are play/pause, the effective Song title (the current User's display title, falling back to canonical title), the Recording's credited-as text, an expand control, and a minimal line progress bar. The expanded state adds artwork and the selected Release Group title, a next button that selects a random playable Recording, links to structured credited Artists when resolvable (plain credited-as text otherwise), and a contextual action: go to the Recording if the current User has saved it, or save it if not. An unsaved preview must retain the Song it originated from; Add finds/creates the canonical Recording and saves it through `user_recording_data` in one tap, with no modal reopening or MusicBrainz confirmation step in between. This is the same immediate-save contract used by Add Recording.
