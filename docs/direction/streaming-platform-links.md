...
Per [ADR-0008](../adr/0008-provider-neutral-music-entities-and-user-data.md), the video ID and result metadata describe a provider item, not the Recording's identity. `youtubeItems` stores the provider snapshot and `recordingYoutubeItems` associates it to Recordings: one Recording can have multiple items, while a long video can support separate Song-scoped Recordings. A Recording can also carry a MusicBrainz identity and other Platform Links.
