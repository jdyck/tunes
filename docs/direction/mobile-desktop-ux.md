...
- **PWA/safe-area**: the app has `display: standalone` in [manifest.json](../../public/manifest.json) for mobile home-screen install. Goal is still to have the app's own color extend behind the notch/Dynamic Island and home indicator, not just a flat fallback color — that didn't come together this round, so for now `html`/`body` are black behind a `merino` wrapper div, which just makes the reserved-but-unreachable safe areas look intentional rather than bleeding real content under them. Revisit later; not abandoned. Two things worth knowing if you pick this back up:
  - The wrapper's `h-full` requires `html, body { height: 100% }` in `globals.css` — removing that silently collapses panes to zero height.
  - The bottom safe area (home indicator) still isn't handled — `GlobalPlayer`'s bar is plain `bottom-0` and likely stops short of the true edge on a real notched device.
