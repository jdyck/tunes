...
**User**:
A person using the app to track their own Songs and Recordings through `songUserData` and `userRecordingData`, with private Artist notes in `artistUserData`. Multi-user support is a design goal; canonical Song, Artist, Recording, and Release Group metadata is shared, while all user-data layers remain private. Each User has a Platform Preference.
