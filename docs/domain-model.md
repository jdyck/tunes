...
The app is named **Standards** (renamed from "Tunes", July 2026 — the repo folder and legacy Supabase project id still say `tunes`). "Standards" as in the repertoire sense: the songs a musician keeps.
...
**song_user_data**:
A User's private layer over a canonical Song: their own notes, their own display title (can diverge from the Song's canonical title — e.g. "I Found a New Baby" instead of the canonical "I've Found a New Baby"), favorite status, tags, when they added it, and the fact that the Song is in their list. Favorites and tags are personal organization and never shared Song metadata. Not a *kind* of Song — it is User-specific data *attached to* a Song, not a Song itself, which is why it is named for data rather than as a "User Song" entity. One undifferentiated list regardless of mastery — gig-ready repertoire, Songs being studied/transcribed, and someday-Songs all live together; no mastery-level silos. Wherever the UI presents a Song to that User, use their display title when present and fall back to the shared title; search within their own list matches both, while cross-User Add Song discovery matches only the shared title. In the UI this is simply presented as "Songs" (the User's list) — `song_user_data` is database/glossary vocabulary, not a UI label. There is at most one private row per User and Song, and owner-scoped Convex authorization keeps its payload private.
...
The primary published credit for a Recording: an ordered attribution containing one or more Artists, each Artist's credited name, and the exact phrases joining them, such as “Kenny Burrell with Coleman Hawkins.” Every part references a canonical Standards Artist even when that Artist has no provider identity. It remains separate from detailed Personnel, and Standards derives its linked display text from the structured Attribution.
_Avoid_: treating the attribution as the complete personnel list

**Attribution Fallback**:
Unresolved legacy, manual, or provider-supplied text shown only when a Recording has no structured Attribution. It preserves useful wording without asserting that an uploader/channel or an unsplit multi-name string is one Artist, and it does not make an Artist reachable in browsing.
_Avoid_: Artist, Recording Attribution

**Recording Personnel**:
The shared set of structured relationships describing which Artists performed on a Recording and what they contributed. Each Artist appears once with all explicitly evidenced instrument, vocal, conductor, orchestra, or generic performer relationships; production work and inferred contributions stay outside Personnel.
_Avoid_: Recording Artist Credit (MusicBrainz uses artist credit for Recording Attribution)
...
**Release Group**:
The master-level concept grouping editions of the same published release. It may be an album, EP, single, or another release type. A MusicBrainz Release Group and Discogs Master are external representations of this concept; a MusicBrainz Release or Discogs Release is one particular edition. Most UI shows only the title, not the words “Release Group,” “Master,” or “Album.” A Release Group has its own Attribution, separate from the Attribution of any Recording it contains. A Recording can point to one Release Group as its recognizable display and artwork context. The pointer is independent of performance date and is not a claim that full publication provenance has been modeled; see [ADR-0008](adr/0008-provider-neutral-music-entities-and-user-data.md).
_Avoid_: Master Album (not every Release Group is an album, and “master” also suggests audio mastering)

**Release Group Attribution**:
The primary published credit for a Release Group: an ordered attribution containing one or more Artists, each Artist's credited name, and the exact phrases joining them. Every part references a canonical Standards Artist even when that Artist has no provider identity. The Attribution belongs to the Release Group rather than its Recordings or one representative edition, so “Ella Fitzgerald and Joe Pass” can remain distinct from a contained Recording's Attribution.
_Avoid_: owner, album owner, treating the rendered attribution as one Artist
